...
const LeftSide = ({ restaurantData }) => {
	...
	useEffect(() => {
		if (restaurantData) {
			const chartData =
				restaurantData?.marketOverview?.monthlyData.flatMap((item) => [
					{
						month: `${item.month}`,
						type: "Total Orders",
	...
	useEffect(() => {
		const data = [
			...
			{
				name: "Từ 18 đến 24",
				age: restaurantData?.customerAgeGroup?.between18And24,
			},
			{
				name: "Từ 25 đến 34",
				age: restaurantData?.customerAgeGroup?.between25And34,
			},
			{
				name: "Từ 35 đến 44",
				age: restaurantData?.customerAgeGroup?.between35And44,
			},
			{
				name: "Từ 45 đến 54",
				age: restaurantData?.customerAgeGroup?.between45And54,
			},
			...
			<Row gutter={[16, 16]} className="d-flex justify-content-center">
					...
					<div className="chart1">
						<div className="d-flex justify-content-space-between align-items-center">
							<div>
								...
								>
									{" "}
									Tổng hợp số đơn hàng trong năm{" "}
								</div>
							</div>
							<div className="select">
								...
								>
									{restaurantData?.yearsContainOrders.map(
										(y) => (
				...
				<Col xs={12} sm={12} md={12} lg={12} xl={12}>
					<div className="chart1" style={{ height: "300px" }}>
						<div className="fs-18 fw-600">Phân tích độ tuổi</div>
						<div>
							<div
								id="chart2"
								...
								.map((c) => (
									<div className="item" key={c?.uid}>
										<div className="d-flex justify-content-center align-items-center">
											...
											<div>
												<div className="fw-600 fs-15 mb-5">
													{" "}
													{c?.name}{" "}
												</div>
