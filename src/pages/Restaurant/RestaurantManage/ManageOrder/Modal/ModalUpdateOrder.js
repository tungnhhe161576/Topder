...
const ModalUpdateOrder = ({open, onCancel, onOk, text, status, userId}) => {
    ...
    const timeChooseTable = dayjs(open?.timeReservation, "HH:mm:ss").format("HH:mm")
    ...
    const getTable = async () => {
        try {
            setLoading(true)
            const res = await UserService.getTable(userId, timeChooseTable, dateChooseTable)
    ...
    const handleUpdate = async () => {
        try {
            ...
            if (open?.statusOrder === 'Pending' && status === 'Confirm' && open?.isTableBooking === false) {
        ...
        <div className="mt-50">
            ...
            >
                <div>
                    ...
                    {
                        open?.statusOrder === 'Pending' && status === 'Confirm' && open?.isTableBooking === false
                            ? 
