import { Button, Form, Input, message, Row, Col, Select, InputNumber } from "antd";
...
const ModalUpdateTable = ({open, onCancel, onOk, userId}) => {
    ...
    useEffect(() => {
        form.setFieldsValue({
            ...open,
            roomId: open?.roomId === null ? 0 : open?.roomId
        })
    }, [open, form])

    const hanleUpdateTable = async () => {
        try {
            setLoading(true)
            ...
            await UserService.updateTable({
                ...
                isBookingEnabled: open?.isBookingEnabled
            })
            message.open({
    ...
    const footer = () => {
        return (
            <div className="d-flex justify-content-center">
                ...
                <Button className="mr-10 fw-600" type="primary" shape='round' onClick={() => hanleUpdateTable()} loading={loading}>
                    Đồng ý
                </Button>
            </div>
        )
                        ...
                        <Col span={12}>
                            ...
                            >
                                <Select allowClear placeholder='Chọn phòng'>
                                    <Option value={0} key={'abc'}>Bàn tự do</Option>
                                    {
                                        rooms.map(r => (
                            ...
                            <Form.Item
                                ...
                                rules={[
                                    { required: true, message: "Vui lòng đặt tên bàn!" },
                                ]}
                            >
