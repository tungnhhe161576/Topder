...
const ModalViewDetail = ({ open, onCancel }) => {
	...
	return (
		...
		>
			<div className="title-type-1">Thông tin nhà hàng</div>
				...
				<Row gutter={[16, 16]} className="">
					...
					<Col span={14} className="d-flex flex-column ml-20">
						<div className="d-flex justify-content-space-between align-items-center w-80 mb-20">
							<div className="fw-600 fs-24">{open?.nameRes}</div>
							<div className="fs-16">
								<span className="fw-500">Chủ: </span>
								<span className="primary fw-500">
									{" "}
									{open?.nameOwner}{" "}
								</span>
							</div>
							...
							<span className="primary">
								{
									category.find(
										(i) =>
											i?.categoryRestaurantId ===
											open?.categoryRestaurantId
									)?.categoryRestaurantName
								}
							</span>
						</div>
