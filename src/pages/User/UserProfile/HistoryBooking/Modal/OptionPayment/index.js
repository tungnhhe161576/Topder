...
const ModalChooseOptionPayment = ({ open, onCancel, user, orderHistory }) => {
	...
	const getReturnFee = async () => {
		try {
			setLoading(true);
			const res = await UserService.returnFee(
				user?.uid,
				open?.restaurantId
			);
			setReturnFee(res);
	...
	const handlePaidPayment = async () => {
		try {
			...
			if (
				formValue?.option === "VNPAY" ||
				formValue?.option === "VIETQR"
			) {
				window.location.href = res;
			}
			orderHistory();
			dispatch(
				updateUserInformation({ walletBalance: wallet?.walletBalance })
			);
			onCancel();
		} catch (error) {
			message.open({
		...
		>
			<SpinCustom spinning={loading}>
				<ModalChooseOptionPaymentContainer>
					<div>
						<div className="mb-20 fs-20 fw-500">Thanh toán</div>
						<div className="">
									...
									>
										<Radio value="VNPAY" className="mb-10">
											<div className="d-flex align-items-center">
												...
												<div className="ml-50 fs-20 fw-500">
													{" "}
													VNPAY{" "}
												</div>
											</div>
					...
					{!!data && (
						<div
							className="mt-10 mb-30 fs-12"
							style={{ fontStyle: "italic" }}
						>
							Lưu ý: Theo chính sách của nhà hàng, đơn hàng của
							bạn sẽ được chiết khấu {returnFee}% giá trị
						</div>
					)}
				</ModalChooseOptionPaymentContainer>
			</SpinCustom>
