				...
				<SpinCustom spinning={loading}>
					<div className='session1'>
						...
						<div className='d-flex justify-content-space-between align-items-center mb-30'>
							...
							<Button style={{width: '150px', height: '40px'}} onClick={() => nav('/restaurant-view')}>
								Xem nhiều hơn
							</Button>
						</div>
						<div className='mt-20'>
							<Row gutter={[30, 24]} className="d-flex justify-content-center">
								{
									data?.newRestaurants?.map((r, index) => (
										<Col key={index} xs={12} sm={12} md={12} lg={6} xl={6}>
											<RestaurantItem data={r} setOpenRequestLogin={setOpenRequestLogin} setOpenModalBooking={setOpenModalBooking} setText={setText} isWishlist={false}/>
										</Col>
									))
								}
							</Row>
					...
					<div className='session3'>
						...
						<div>
							<Row gutter={[30, 24]} className="d-flex justify-content-center">
								{
									data?.topBookingRestaurants?.map((r, index) => (
										<Col key={index} xs={12} sm={12} md={12} lg={6} xl={6}>
											<RestaurantItem data={r} setOpenRequestLogin={setOpenRequestLogin} setOpenModalBooking={setOpenModalBooking} setText={setText} isWishlist={false}/>
										</Col>
									))
