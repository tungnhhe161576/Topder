...
const ModalCalFee = ({open, onCancel, restaurantId, userId, totalPrice, form}) => {
    ...
    const getAllDiscount = async () => {
        try {
            setLoading(true)
            const res = await UserService.getAllDiscount(restaurantId, userId, totalPrice)
            setDiscounts(res)
        } catch (error) {
            console.log(error);
        } finally {
                ...
                <SpinCustom spinning={loading}>
                    <div>
                        ...
                        <div>
                            ...
                            <div>
                                <span className="fs-16 fw-500">Số điện thoại: </span>
                                <span> {open?.phoneReceiver} </span>
                            </div>
                            <div>
                                <span className="fs-16 fw-500">Thời gian: </span>
                                <span> {dayjs(open?.timeReservation).format('HH:mm')} </span>
                            </div>
                            <div>
                                <span className="fs-16 fw-500">Ngày/Tháng/Năm: </span>
                                <span> {dayjs(open?.dateReservation).format('DD-MM-YYYY')} </span>
                            </div>
                            <div>
                                <span className="fs-16 fw-500">Số người lớn: </span>
                                <span> {open?.numberPerson} </span>
                            </div>
                            <div>
                                <span className="fs-16 fw-500">Số trẻ em: </span>
                                <span> {open?.numberChild} </span>
                            </div>
                            <div>
                                <span className="fs-16 fw-500">Yêu cầu: </span>
                                <span> {open?.contentReservation ? open?.contentReservation : 'Không có'} </span>
                            </div>
                            <div>
                                <span className="mt-20 fs-16 fw-500">Tổng tiền:  </span>
